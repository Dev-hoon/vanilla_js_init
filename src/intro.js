...
// let value = 1;
// let value = 2;
//let 은 위와 같이 재 정의가 불가능 함으로, 사용된 변수 인지를 쉽게 확인하여
// 관리 할 수 있가.
// 하지만 var의 경우는
var hoyeon = "멍청이";
var shoyeon = "양아치";
...
// 하지만 var 이 없어지는 것은 안된다. 여러가지의 이유가 있다.
// 그 이유가 무엇인가? <과제 1>

//상수
const a = 1; //는 상수로서 변경이 불가능 한 값이다. 처음 선언한 값이
//그대로 유지된다.
//a=3; //과 같이 변경 하려고 시도시 에러가 뜨는것을 볼수 있다.
...
let good = true;
let loading = false;
// bool 값 또는 boolean 이라고 불리운다,
let friend = null;
let criminal = undefined;
// 없는 값 즉 빈값을 말하는 것 null은 의도된 빈값을 표현하고,
// undefined는 의도되지 않은 빈값을 얘기한다.

//연산자
//= 대입 연산자 a=3 -> a변수에 3이라는 값을 대입하여라
let b = 1 + 2;
console.log(b);
...
f = false || false;
console.log(f);
// 위 논리 연산자는 결과를 보기전에결과를 출분히 생각해보고 예측할 필요가 있음
